-- Seed realistic placement exam data for analytics (FAKE DATA VERSION)
-- This version temporarily disables foreign key constraints to allow fake user IDs
-- WARNING: Only use this for testing! Re-enable constraints afterward.
...
BEGIN
    ...
    ALTER TABLE public.placement_logs DROP CONSTRAINT IF EXISTS placement_logs_user_id_fkey;
    
    RAISE NOTICE 'Foreign key constraint temporarily disabled for testing';
    
    -- Insert diverse placement exam results with fake user IDs
    FOR i IN 1..25 LOOP
        user_id := sample_user_ids[((i - 1) % 20) + 1];
    ...
    ALTER TABLE public.placement_logs 
    ADD CONSTRAINT placement_logs_user_id_fkey 
    FOREIGN KEY (user_id) REFERENCES auth.users(id) ON DELETE CASCADE;
    
    RAISE NOTICE 'Successfully inserted 25 placement exam records with fake data';
    RAISE NOTICE 'Foreign key constraint re-enabled';
    RAISE NOTICE 'WARNING: This data uses fake user IDs - only for testing!';
END $$;